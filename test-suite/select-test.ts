...
const invalidSelect2 = await dorm
.select()
.from('profile')
.where('_id=1')
.then((data: any) => {
...
Deno.test(`all queries to be valid in "SELECT" method:`,() => {
  assertEquals(invalidSelect2, false, `Error:INVALID query found!!!! It should  return an error for invalid query request from Postgres.`) 
})
